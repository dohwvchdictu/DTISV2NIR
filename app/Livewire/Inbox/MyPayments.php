...
class MyPayments extends Component
{
    ...
    public function mount()
    {
         ...
         *
         * A one-month default silently hid every payment older than that - including
    ...
    /**
     * Which documents belong on screen right now: this office's payments, plus the
     * active filters.
     *
    ...
    public function forward()
    {
        ...
        if (empty($documentIds)) {
            ...
            $this->alert('warning', 'Nothing to forward. The selected payment(s) are no longer available.', [
                'position' => 'top-end',
                'timer' => 10000,
                'toast' => true
            ]);
         ...
         * They cost two seconds per payment plus two per attachment, and inside a
        ...
        DB::transaction(function () use ($documentIds, $data, $lookUpOffice, $forwardedActionId, $receivingActionId) {
            foreach ($documentIds as $item) {
                ...
                $doc_type = $document->is_bundle ? 'Bundle Payment' : 'Payment';
        ...
        return $this->flash('success', 'Payment successfully forwarded!', [
            'position' => 'top-end',
            'timer' => 10000,
            'toast' => true
        ], '/my-payments');
    ...
    public function updatedSelectAll($value)
    {
         ...
         * This used to ignore the search box entirely, selecting payments that were
